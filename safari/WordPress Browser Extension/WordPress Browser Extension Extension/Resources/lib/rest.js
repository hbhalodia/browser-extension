...
(function () {
   ...
   * The capabilities map refines the decision when it's available.
   */
  function canEditCurrent(ctx, user) {
    ...
    if (caps) {
      switch (ctx.pageType) {
        case 'single':
          // Pages need edit_pages; posts, attachments, and most CPTs map to
          // the edit_posts family (the default capability_type). We can't
          // know an arbitrary CPT's custom caps from allcaps, so edit_posts
          // || edit_pages is the practical floor — it gates subscribers (who
          // have neither) while leaving capable roles enabled.
    ...
    // No capabilities map. For a singular object, the admin bar WordPress
    // rendered is authoritative: a logged-in user with the bar present but no
    // Edit link cannot edit this object. We scope this to `single` because
    // core reliably renders the bar's Edit link for editable posts/pages/CPTs;
    // term/author archives are less consistent, so we stay unknown there.
